...
public class SbeScreen extends Screen {
    ...
    private SbeScreen(Screen parent, String category) {
    ...
    protected void rebuildWidgets() {
        ...
        for (int i = 0; i < CATEGORIES.length; i++) {
            ...
            addRenderableWidget(new CategoryButton(
                    ...
                    () -> Minecraft.getInstance().gui.setScreen(new SbeScreen(this.parent, selected))
            ));
        }
    ...
    private void buildGui() {
        ...
        addRenderableWidget(new ActionButton(x + 12, y + 31, 92, 19,
                "OPEN EDITOR", false,
                () -> Minecraft.getInstance().gui.setScreen(new SbeScreen(this, "GUI"))));
        y += 63;
    ...
    private void buildFarming() {
        ...
        y = addSection("Harvest Feast", "Track Harvest Feast drops.",
                ...
                () -> {
                    ...
                    Minecraft.getInstance().gui.setScreen(new SbeScreen(this.parent, "Farming RNG"));
                },
            ...
            for (int i = 0; i < HARVEST_FEAST_DROPS.length; i++) {
                ...
                addItemToggle(item,
                        ...
                        () -> toggle(() -> SkyblockExtrasClient.CONFIG.harvestFeastDrops.put(
                                item, !SkyblockExtrasClient.CONFIG.harvestFeastDrops.getOrDefault(item, true))));
            }
            y += 9 * rowH + 5;
        }
    ...
    private int addSection(String title, String description, BooleanSupplier enabled,
        ...
        addRenderableWidget(new ActionButton(x + 6, y + 4, 210, 22,
    ...
    private class LabelWidget extends AbstractWidget {
        ...
        LabelWidget(int x, int y, String text, int color) {
            ...
            this.text = text; this.color = color; this.active = false;
        }
        @Override protected void extractWidgetRenderState(GuiGraphicsExtractor g, int mx, int my, float d) {
    ...
    private class CategoryButton extends AbstractWidget {
        ...
        @Override protected void extractWidgetRenderState(GuiGraphicsExtractor g, int mx, int my, float d) {
            ...
            if (selected) g.fill(getX(), getY(), getX() + 3, getY() + height, 0xFFC276FF);
            int tw = SbeScreen.this.font.width(text);
            g.text(SbeScreen.this.font, text, getX() + (width - tw) / 2, getY() + 10,
    ...
    private class ActionButton extends AbstractWidget {
        ...
        @Override protected void extractWidgetRenderState(GuiGraphicsExtractor g, int mx, int my, float d) {
            ...
            g.text(SbeScreen.this.font, getMessage(), getX() + (width - tw) / 2,
                    getY() + (height - 8) / 2, accent ? 0xFFEBDCF4 : 0xFFD9D9E0, false);
        }
        @Override public void onClick(MouseButtonEvent event, boolean doubleClick) { action.run(); }
        @Override protected void updateWidgetNarration(NarrationElementOutput b) { defaultButtonNarrationText(b); }
    }
    ...
    private class ToggleButton extends AbstractWidget {
        private final String label; private final BooleanSupplier enabledSupplier; private final Runnable action;
        private float progress; private long lastFrameNanos;
        ToggleButton(int x, int y, int w, int h, String label, BooleanSupplier enabledSupplier, Runnable action) {
            super(x, y, w, h, Component.literal(label)); this.label = label; this.enabledSupplier = enabledSupplier; this.action = action;
            this.progress = enabledSupplier.getAsBoolean() ? 1.0f : 0.0f; this.lastFrameNanos = System.nanoTime();
        }
        @Override protected void extractWidgetRenderState(GuiGraphicsExtractor g, int mx, int my, float d) {
            ...
            if (!label.isEmpty()) {
                ...
                int tw = SbeScreen.this.font.width(text);
                g.text(SbeScreen.this.font, text, getX() - tw - 8, getY() + 6, 0xFFBFC0C9, false);
            }
        }
        @Override public void onClick(MouseButtonEvent event, boolean doubleClick) { action.run(); }
        @Override protected void updateWidgetNarration(NarrationElementOutput b) { defaultButtonNarrationText(b); }
    ...
    private static int lerpColor(int a, int b, float t) {
        ...
        int ca = aa + Math.round((ba - aa) * t), cr = ar + Math.round((br - ar) * t);
        int cg = ag + Math.round((bg - ag) * t), cb = ab + Math.round((bb - ab) * t);
