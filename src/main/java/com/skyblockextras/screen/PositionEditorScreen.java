...
public class PositionEditorScreen extends Screen {
    ...
    private int previewX() {
        ...
        return Math.max(10, Math.min(width - previewW - 10, c.petX));
    }

    private int previewY() {
        SbeConfig c = SkyblockExtrasClient.CONFIG;
        int previewH = Math.max(72, Math.round(72 * c.petScale));
        return Math.max(105, Math.min(height - previewH - 90, c.petY + 95));
    }
    ...
    public boolean mouseDragged(MouseButtonEvent event, double deltaX, double deltaY) {
        if (dragging && event.button() == 0) {
            ...
            int newY = (int) Math.round(event.y() - dragOffsetY - 95);

            int maxX = Math.max(0, width - previewWidth() - 10);
            int maxY = Math.max(0, height - previewHeight() - 105);
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        int border = dragging ? 0xFFE0A7FF : (mouseX >= previewX && mouseX <= previewX + previewW
                && mouseY >= previewY && mouseY <= previewY + previewH ? 0xFFC276FF : 0xFF8A42D2);
        ...
        g.text(font, dragging ? "Dragging..." : "Click and drag me", previewX + 12, previewY + 63,
                dragging ? 0xFFC276FF : 0xFF777984, false);
